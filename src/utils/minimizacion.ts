...
export function minimizarAFD(afd: Automata) : Automata{
    ...
    while(cambios){
        ...
        for(let i = 0; i < subGrupos.length; i++){
            ...
            for(let caracter of alfabeto){
                let primerSubGrupo = getSubGrupo(subGrupos,subGrupo[0].adyacentes[caracter]?.[0])
                for(let nodo of subGrupo){
                    if (primerSubGrupo != getSubGrupo(subGrupos,nodo.adyacentes[caracter]?.[0])){
                        cambios = true;
    ...
    for(let i=0; i<nuevosNodos.length;i++){
        ...
        alfabeto.forEach(caracter => {
            let index = getSubGrupoIndex(subGrupos,subGrupo[0].adyacentes[caracter]?.[0])
            nodo.agregarArista(nuevosNodos[i],caracter)
        })
    }
    console.log(minAFD)
    return minAFD;
